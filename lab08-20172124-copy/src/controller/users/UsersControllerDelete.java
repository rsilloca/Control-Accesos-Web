			...
			if (userSearch.isEmpty()) {
				...
				if (resourceSearch.isEmpty()) {
					...
					if (accessSearch.isEmpty()){
						...
						if(userSearch.get(0).getId() == Long.parseLong(request.getParameter("id"))){
							RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Views/Information/DeniedEliminate.jsp");
							dispatcher.forward(request, response);
						} else {
